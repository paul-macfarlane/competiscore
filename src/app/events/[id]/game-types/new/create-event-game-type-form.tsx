...
type CreateEventGameTypeFormProps = {
  eventId: string;
};

export function CreateEventGameTypeForm({
  eventId,
}: CreateEventGameTypeFormProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [selectedTemplate, setSelectedTemplate] = useState<string | null>(null);

  const form = useForm<CreateEventGameTypeInput>({
    resolver: zodResolver(createEventGameTypeSchema),
    defaultValues: {
      eventId,
      name: "",
      description: "",
  ...
  const onSubmit = (values: CreateEventGameTypeInput) => {
    startTransition(async () => {
      const result = await createEventGameTypeAction(values);
      if (result.error) {
        if (result.fieldErrors) {
          Object.entries(result.fieldErrors).forEach(([field, message]) => {
            form.setError(field as keyof CreateEventGameTypeInput, { message });
          });
        } else {
