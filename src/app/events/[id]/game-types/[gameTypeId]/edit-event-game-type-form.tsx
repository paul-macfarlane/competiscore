...
export function EditEventGameTypeForm({
  gameType,
  eventId,
}: EditEventGameTypeFormProps) {
  ...
  const form = useForm<UpdateEventGameTypeFormValues>({
    resolver: zodResolver(updateEventGameTypeSchema),
    defaultValues: {
  ...
  const onSubmit = (values: UpdateEventGameTypeFormValues) => {
    startTransition(async () => {
      const result = await updateEventGameTypeAction(
      ...
      if (result.error) {
        if (result.fieldErrors) {
          Object.entries(result.fieldErrors).forEach(([field, message]) => {
            form.setError(field as keyof UpdateEventGameTypeFormValues, {
              message,
            });
          });
        } else {
  ...
  const handleArchive = () => {
    startTransition(async () => {
      const result = await archiveEventGameTypeAction({
        gameTypeId: gameType.id,
      });
      ...
      if (result.error) {
        ...
        router.push(`/events/${eventId}/game-types`);
      }
    });
  };
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteEventGameTypeAction({
        gameTypeId: gameType.id,
      });
