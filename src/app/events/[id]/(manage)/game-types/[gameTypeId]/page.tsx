...
import { EditEventGameTypeForm } from "./edit-event-game-type-form";

type PageProps = {
  params: Promise<{ id: string; gameTypeId: string }>;
};

export default async function EventGameTypeDetailPage({ params }: PageProps) {
  const { id: eventId, gameTypeId } = await params;
  ...
  const [gameTypeResult, eventResult] = await Promise.all([
    getEventGameType(session.user.id, gameTypeId),
  ...
  const canManage = canPerformEventAction(
    event.role,
  ...
  return (
    <div className="space-y-6">
      ...
      {canManage && (
        <EditEventGameTypeForm gameType={gameType} eventId={eventId} />
      )}
    </div>
  );
}
